...
@testset "Data Type Validation Tests" begin
    @testset "Node Data Type Tests" begin
        ...
        node_no_tags = Node(LatLon(0.0, 0.0), nothing, nothing)
    ...
    @testset "Data Type Edge Cases" begin
        ...
        extreme_node = Node(LatLon(90.0, 180.0), Dict("extreme" => "true"), nothing)
        @test extreme_node.latlon.lat == 90.0
        @test extreme_node.latlon.lon == 180.0

        # Test with negative extreme coordinates
        negative_extreme_node = Node(LatLon(-90.0, -180.0), Dict("negative" => "true"), nothing)
        ...
        node_many_tags = Node(LatLon(54.0, 9.0), many_tags, nothing)
    ...
    @testset "Data Type Validation Errors" begin
        ...
        try
            relation = Relation(
                [1, 2], ["node"], ["role1", "role2"], Dict("test" => "mismatch"), nothing
            )
