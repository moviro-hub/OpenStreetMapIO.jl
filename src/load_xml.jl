...
function parse_bounds(xmlnode::XML.Node)::BBox
    ...
    return BBox(
        parse(Float32, attrs["minlat"]),
        parse(Float32, attrs["minlon"]),
        parse(Float32, attrs["maxlat"]),
        parse(Float32, attrs["maxlon"]),
    )
end
...
function parse_node(xmlnode::XML.Node)::Tuple{Int64, Node}
    ...
    latlon = LatLon(parse(Float32, attrs["lat"]), parse(Float32, attrs["lon"]))
